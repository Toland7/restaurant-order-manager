...
const AuthPage = () => {
    ...
    const handleSignUp = async (e) => {
      ...
      try {
        ...
        if (error) throw error;
        if (data.user) {
          toast.success('Registrazione completata! Controlla la tua email per confermare l\'account. Un amministratore approverà il tuo accesso a breve.');
          setIsLogin(true);
        }
      } catch (error) {
              ...
              <form onSubmit={isLogin ? handleLogin : handleSignUp} className="space-y-4">
                ...
                {isLogin && (
                  <div className="text-right">
                    <button type="button" onClick={() => setShowResetPassword(true)} className="text-xs text-blue-500 hover:text-blue-600">Password dimenticata?</button>
                  </div>
                )}
