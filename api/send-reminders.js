    ...
    const allPromises = Object.values(groupedReminders).map(async (group) => {
      ...
      if (group.length > 1) {
        ...
        payload = JSON.stringify({
          ...
          data: {
            url: `/create-order?reminder_ids=${group
              .map((r) => r.id)
              .join(",")}&user_id=${user_id}&flowInitialStep=review`,
            forceReauth: true,
          },
        });
      } else {
        ...
        payload = JSON.stringify({
          ...
          body:
            reminder.reminder_type === "prefilled"
      ...
      const sendPromises = subscriptions.map((sub) =>
        webpush.sendNotification(sub.subscription, payload).catch((err) => {
          console.error(
            `Error sending push notification to one device for user ${user_id}:`,
            err
          );
      ...
      if (isAnySuccess) {
        ...
        const notificationData = {
          ...
          title:
            group.length > 1
              ? `Promemoria per ${group.length} ordini`
              : `Promemoria Ordine: ${firstReminder.suppliers.name}`,
          message:
            group.length > 1
              ? `Hai ${group.length} ordini programmati pronti per essere inviati.`
              : `È ora di fare l'ordine per ${firstReminder.suppliers.name}!`,
          type: "info",
          reminder_id: firstReminder.id,
          data:
            group.length > 1
              ? {
                  reminder_ids: group.map((r) => r.id),
                  user_id: user_id,
                  url: `/create-order?reminder_ids=${group
                    .map((r) => r.id)
                    .join(",")}&flowInitialStep=review`,
                }
              : {
                  reminder_id: firstReminder.id,
                  user_id: user_id,
                  url: `/create-order?reminder_id=${firstReminder.id}&flowInitialStep=review`,
                },
        };
